...
/*   Updated: 2019/10/27 20:20:55 by vellery-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_zarr(t_fdf *fdf, int fd, char *filename)
{
	...
	check_malloc(fdf->zarr = (double**)malloc(sizeof(double*)\
					* fdf->full->hei));
	check_malloc(fdf->static_z = (int**)malloc(sizeof(int*) * fdf->full->hei));
	i = -1;
	while (++i < fdf->full->hei)
	{
		...
		check_malloc(fdf->zarr[i] = (double*)malloc(sizeof(double)\
					* fdf->full->wid));
		check_malloc(fdf->static_z[i] = (int*)malloc(sizeof(int) * fdf->full->wid));
		j = -1;
...
double			get_max_z(t_fdf *fdf)
{
	...
	fdf->col.z_max = (int)mx;
	fdf->col.z_min = (int)mn;
	fdf->col.dist = fdf->col.z_max - fdf->col.z_min;
	mx = fabs(mx) / 3.0 * 2.0;
	mn = fabs(mn) / 3.0 * 2.0;
	return (mx > mn ? mx : mn);
}
