...
/*   Updated: 2019/10/27 22:54:02 by vellery-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_x(t_line ln, t_map *fdf, t_color color /*int color*/)
{
	while (ln.x < ln.xe)
	{
		if (!(ln.x + fdf->offx < 0 || ln.x + fdf->offx > HEI ||
			...
			mlx_pixel_put(fdf->mlx, fdf->win, ln.x + fdf->offx,
						ln.y + fdf->offy, get_color(ln, color));
		ln.x++;
		if (ln.sx < 0)
			ln.sx += 2 * ln.abs_dy;
		else
...
static void	draw_y(t_line ln, t_map *fdf, t_color color /*int color*/)
{
	while (ln.y < ln.ye)
	{
		if (!(ln.x + fdf->offx < 0 || ln.x + fdf->offx > HEI ||
...
void		draw_line(t_map *fdf, t_coor cr1, t_coor cr2, t_color color /*int color*/)
{
	t_line		ln;
	int			tmp;
